...
public class CountriesCampaignScreen extends AbstractScreen<CountriesScreenManager> {
    ...
    private CountriesSettings flagsSettings;

    public CountriesCampaignScreen() {
        initFlagsSettings();
        CountriesContainers.init();
    }
    ...
    public void buildStage() {
        ...
        addActor(table);
        CountriesContainers.setBackgroundDiff(flagsSettings, getBackgroundStage());
    }
    ...
    private void initFlagsSettings() {
        if (flagsSettings == null) {
            ...
            if (StringUtils.isNotBlank(json)) {
                flagsSettings = new Gson().fromJson(json, CountriesSettings.class);
            } else {
                flagsSettings = new CountriesSettings();
            }
        }
    }
    ...
    private void addButtonToTable(Table table, FlagsCampaignLevelEnum campaignLevel) {
        ...
        int leftCountriesToPlay = CountriesContainers.getAllQuestions(campaignLevel).size();
        if (campaignStoreService.isQuestionAlreadyPlayed(campaignLevel.getName())) {
            ...
            containerTable
                    .add(CountriesContainers.createFlagsCounter(leftCountriesToPlay, labelWidth, MainResource.popup_background))
                    .padTop(MainDimen.vertical_general_margin.getDimen() * 3)
    ...
    private MyButton createCategButton(final CampaignLevel campaignLevel) {
        ...
        categBtn.addListener(new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                List<Question> allQuestions = CountriesContainers.getAllQuestions(campaignLevel);
