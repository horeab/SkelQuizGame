    ...
    private void processPlayedQuestions() {
        for (GameQuestionInfo gameQuestionInfo : gameContext.getCurrentUserGameUser().getAllQuestionInfos()) {
            if (gameQuestionInfo.getStatus() == GameQuestionInfoStatus.LOST) {
                ...
                if (gameQuestionInfo.getStatus() == GameQuestionInfoStatus.WON && !campaignStoreService.isQuestionAlreadyPlayed(ConthistoryContainers.getQuestionId(question.getQuestionLineInQuestionFile(), question.getQuestionCategory(), question.getQuestionDifficultyLevel()))) {
                    campaignStoreService.putQuestionPlayed(ConthistoryContainers.getQuestionId(question.getQuestionLineInQuestionFile(), question.getQuestionCategory(), question.getQuestionDifficultyLevel()));
                    int questionLineInQuestionFile = question.getQuestionLineInQuestionFile();
                    if (ConthistoryContainers.isJudetFound(questionLineInQuestionFile)) {
