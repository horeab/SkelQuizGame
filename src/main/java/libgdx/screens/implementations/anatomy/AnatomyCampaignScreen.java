...
public class AnatomyCampaignScreen extends AbstractScreen<HangmanScreenManager> {
    ...
    private Table createAllTable() {
        ...
        Image titleBackr = GraphicUtils.getImage(AnatomySpecificResource.title_background);
    ...
    private Table createCampaignButtons() {
        Table table = new Table();
        int totalLevels = AnatomyCampaignLevelEnum.values().length;
        for (int i = 0; i < totalLevels; i++) {
            Integer category = CampaignLevelEnumService.getCategory(AnatomyCampaignLevelEnum.values()[i].name());
    ...
    private Table createCampaignBtn(int cat) {
        ...
        if (btnUnlocked) {
            ...
            if (isStar) {
                ...
                table.add(GraphicUtils.getImage(AnatomySpecificResource.star)).width(starDimen).height(starDimen);
            }
            btn.addListener(new ClickListener() {
                @Override
                public void clicked(InputEvent event, float x, float y) {
                    CampaignLevel campaignLevel = CampaignLevelEnumService.getCampaignLevelForDiffAndCat(AnatomyQuestionDifficultyLevel._0, AnatomyQuestionCategoryEnum.valueOf("cat" + cat));
                    AnatomyGame.getInstance().getScreenManager().showCampaignGameScreen(new GameContextService().createGameContext(new CampaignLevelEnumService(campaignLevel).getQuestionConfig(TOTAL_QUESTIONS)), campaignLevel);
                }
            });
        }
        return table;
